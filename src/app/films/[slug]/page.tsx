        ...
        <div className={styles.filmDetails}>
          ...
          <div className={styles.infoSection}>
            ...
            <div className={styles.metadata}>
              ...
              {film.director && (
                <span className={styles.director}>Réalisé par {film.director}</span>
              )}
              {film.saga && (
                  ...
                  {film.links.map((link) => (
                    <div key={link.id} className={styles.episodeCard}>
                      <div className={styles.episodeInfo}>
                        <h3 className={styles.episodeTitle}>{link.podcast.title}</h3>
                        <p className={styles.episodeDate}>
                          {new Date(link.podcast.pubDate).toLocaleDateString('fr-FR', {
                            day: 'numeric',
                            month: 'long',
                            year: 'numeric'
                          })}
                        ...
                        {link.podcast.duration && (
                          <p className={styles.episodeDuration}>
                            Durée : {Math.floor(link.podcast.duration / 60)}:{(link.podcast.duration % 60).toString().padStart(2, '0')}
                          </p>
                        )}
                      </div>
                      <a
